...
// $address = str_replace(' ', '+', trim($dealer_info['street'])).'+'.str_replace(' ', '+', trim($dealer_info['city'])).'+'.str_replace(' ', '+', trim($dealer_info['state']));
// $url = "http://maps.google.com/maps/api/geocode/json?address=$address&sensor=false®ion=UnitedStates";
// $ch = curl_init();
...
// queryDB("UPDATE dealers SET `lat`='".$response_a->results[0]->geometry->location->lat."', `long`='".$response_a->results[0]->geometry->location->lng."' WHERE id='".$dealer_info['id']."' LIMIT 1", 'e');
