      ...
      <div class="container">
        ...
        <div class="row">
            <div class="col-sm-12">
                ...
                //define today so we can group shows
                $today = mktime(date("H"),date("i"),date("s"),date("m"),date("d"),date("Y"));
                $sql = "SELECT * FROM tblUserShow
                INNER JOIN tblTripShow ON tblUserShow.intTripShowID = tblTripShow.intTripShowID
                INNER JOIN tblTrip ON tblTripShow.intTripID = tblTrip.intTripID
                WHERE tblTrip.isArchived=0 AND tblUserShow.intUserID=" . $_SESSION["userID"] . " ORDER BY dateJoined DESC";
                ...
                if($numShows > 0) {
                    while($show = mysql_fetch_array($result)) {
                        ...
                        if(strtotime($show["dateBegin"]) < $today && strtotime($show["dateEnd"]) > $today) {  ?>


                        <?php } // end show is currently happening
                        // show is in the future
                        else if($show["dateBegin"] < $today) { ?>
                            <div class="panel panel-default">
                              <div class="panel-body">
                                <div class="col-md-9">
                                    <h2><?= $show["strTitle"]; ?></h2>
                                    <div class="tripMeta">
                                        <?= date("l, F j, Y,", strtotime($show["dateBegin"])) . " at " . date("g:i A", strtotime($show["dateBegin"])) ?> EST (Duration: <?= timeDiff( date("G:i", strtotime($show["dateBegin"])), date("G:i", strtotime($show["dateEnd"]))) ?> minutes)
                                    </div>
                                    <div class="tripActions">
                                        <a href="<?= $show["strArchiveURL"] ?>" class="btn btn-success"><i class="fa fa-video-camera"></i> Watch Show</a>
                                        <a href="<?= $show[""] ?>" class="btn btn-default"><i class="fa fa-list"></i> View Scientist Answers</a>
                                    </div>
                                    <p class="tripDescription"><?= $show["strDescription"]; ?></p>

                                    <?= $show["strTeacherGuide"]; ?> <br>

                                     Output questions <br>
                                    Show question field if live
                                </div>
                                <div class="col-md-3">
                                    <img class="hidden-xs hidden-sm img-responsive" src="<?= $show["strPhoto400"]?>" alt="sdf">
                                </div>
                        ...
                        <?php } // end show is in the future
                        else { // show is in the past ?>


                        <?php } // end show is in the past
                    } // end while
                } // numshows > 0
                else {
                    // no shows
...
<div class="col-lg-12 col-md-12">
        ...
        <?php       }
                    ...
                    $sqlshow = "SELECT * FROM tblTripShow WHERE intTripID=" . $trip["intTripID"] . " AND dateEnd > NOW() ORDER BY dateBegin ASC LIMIT 1";
                    $resultshow = mysql_query($sqlshow);
                    $numShowsUpcoming = mysql_num_rows($resultshow);
                    $show = mysql_fetch_array($resultshow);

                    $showLength = strtotime($show["dateEnd"]) - strtotime($show["dateBegin"]);
                    $difference = strtotime($show["dateEnd"]) - strtotime('now');
                    ...
                    ################################-->
                        ...
                        <div class='panel-body'>
                            <div class="row">
                                ...
                                <?php if(strtotime($show["dateBegin"]) < $today && strtotime($show["dateEnd"]) > $today) { ?>
                                <br /><br />
                                <div class="panel panel-success">
                                    ...
                                    <div class="panel-body">
                                        ...
                                        <form class="has-success form" action="functions/doQuestion.php" method="post" role="form">
                                          ...
                                          <input type="hidden" name="showID" value="<?= $show["intTripShowID"]?>" />
                                ...
                                <div class="col-lg-9">
                                    ...
                                    <?php
                                    if(!empty($show["dateBegin"])) {
                                    ?>
                                    <div class="alert alert-info">
                                    <strong>Next Show</strong>: <?= date("l, F j, Y,", strtotime($show["dateBegin"])) . " at " . date("g:i A", strtotime($show["dateBegin"])) ?> EST (Duration: <?= timeDiff( date("G:i", strtotime($show["dateBegin"])), date("G:i", strtotime($show["dateEnd"]))) ?> minutes)
                                    <?php if(!empty($show["strLiveURL"])) { echo "<br /><strong>Live URL:</strong> <a class='alert-link' href='" . $show["strLiveURL"] . "'>" . $show["strLiveURL"] . "</a>";} ?>
                                    </div>
                                    <?php } ?>
